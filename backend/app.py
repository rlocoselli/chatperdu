...
def create_app(test_config=None):
    ...
    def _safe_redirect_to(value):
      if not value: return f"{request.scheme}://{request.host}"
      p=urllib.parse.urlparse(value)
      if p.scheme not in ('http','https') or not p.netloc: return f"{request.scheme}://{request.host}"
      return value

    @app.get('/api/auth/google/start')
    def google_start():
      client_id=app.config.get('GOOGLE_CLIENT_ID','')
      if not client_id: return jsonify(error='Google OAuth non configuré'),503
      mode=request.args.get('mode','login')
      if mode not in ('login','signup'): mode='login'
      redirect_to=_safe_redirect_to(request.args.get('redirect_to',''))
      callback_url=f"{request.scheme}://{request.host}/api/auth/google/callback"
      state=jwt.encode({'nonce':secrets.token_hex(16),'mode':mode,'redirect_to':redirect_to,'exp':now()+timedelta(minutes=15)},app.config['JWT_SECRET'],algorithm='HS256')
      params=urllib.parse.urlencode({'client_id':client_id,'redirect_uri':callback_url,'response_type':'code','scope':'openid email profile','state':state,'access_type':'online'})
      return redirect(f"https://accounts.google.com/o/oauth2/v2/auth?{params}")

    @app.get('/api/auth/google/callback')
    def google_callback():
      error=request.args.get('error'); code=request.args.get('code'); state_token=request.args.get('state','')
      try: state=jwt.decode(state_token,app.config['JWT_SECRET'],algorithms=['HS256'])
      except jwt.PyJWTError: return redirect('/#google_error=invalid_state')
      redirect_to=_safe_redirect_to(state.get('redirect_to',''))
      if error or not code: return redirect(f"{redirect_to}/#google_error={urllib.parse.quote(error or 'cancelled')}")
      callback_url=f"{request.scheme}://{request.host}/api/auth/google/callback"
      try:
        tr=_http.post('https://oauth2.googleapis.com/token',data={'code':code,'client_id':app.config['GOOGLE_CLIENT_ID'],'client_secret':app.config['GOOGLE_CLIENT_SECRET'],'redirect_uri':callback_url,'grant_type':'authorization_code'},timeout=10)
        tr.raise_for_status()
        access_token=tr.json()['access_token']
        ui=_http.get('https://www.googleapis.com/oauth2/v3/userinfo',headers={'Authorization':f'Bearer {access_token}'},timeout=10)
        ui.raise_for_status()
        info=ui.json()
      except Exception: app.logger.exception('Google OAuth failed'); return redirect(f"{redirect_to}/#google_error=auth_failed")
      email=info.get('email','').lower().strip(); name=info.get('name') or info.get('given_name') or email.split('@')[0]
      if not email: return redirect(f"{redirect_to}/#google_error=no_email")
      user=User.query.filter_by(email=email).first()
      if not user:
        user=User(email=email,name=name[:80],password_hash=generate_password_hash(secrets.token_hex(32))); db.session.add(user);db.session.flush();preferences(user.id);db.session.commit()
